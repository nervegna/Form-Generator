...
class Base_Textarea extends Base_Element
{
	 ...
	 */
	public function __construct($label="", $name="", $value="", $cols=15, $rows=5, $required=false, $validators=null, $description=null){
	...
	public function __toString(){
		...
		// Declarations
		$format 			= TAB.TAB . '<li id="li_' . $this->attr_values['name'] . '"%s>' . NEWLINE.TAB.TAB.TAB . '<label%s %s for="%s">%s%s</label>' . NEWLINE.TAB.TAB.TAB . '<textarea%s %s>%s</textarea>' . NEWLINE.TAB.TAB . '%s%s</li>';
		...
		// Vallue attribute
		$value_string = "";
		if(isset($this->attr_values['value']) && $this->attr_values['value']){
			$value_string = $this->attr_values['value']; 
		}
		...
		return sprintf($format, $li_class, $required, $label_id, $this->attr_values['name'], $this->label, $requiredcontent, $required, $attribute_string, $value_string, $description, $error);
